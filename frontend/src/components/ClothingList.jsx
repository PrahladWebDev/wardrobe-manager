...
const ClothingList = ({ token }) => {
  ...
  const handleDelete = async (id) => {
    try {
      await axiosInstance.delete(`/clothing/${id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleFavorite = async (id, isFavorite) => {
    try {
      await axiosInstance.put(`/clothing/${id}`, { isFavorite: !isFavorite }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleWear = async (id) => {
    try {
      ...
      await axiosInstance.put(`/clothing/${id}`, { wearCount: current.wearCount + 1, lastWorn: now }, { headers: { Authorization: `Bearer ${token}` } });
